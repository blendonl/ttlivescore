...
export class MatchDetailsComponent {
  ...
  constructor(
    ...
    route.url.subscribe(
      (data) =>
        (this.matchId = data
          .toString()
          .slice(data.toString().lastIndexOf('/'))),
    );

    matchService
      .getMatchById(Number.parseInt(this.matchId))
  ...
  getPoints() {
    this.matchService
      .getMatchPoints(Number.parseInt(this.matchId))
      .subscribe((data) => {
        this.points.push(...data);
        this.getAllSets();
      });

    this.matchService.getRealTimeMatchPoints(
      Number.parseInt(this.matchId),
    ).onmessage = (data) => {
  ...
  getAllSets() {
    let setIndexes = this.points
      .map((point, index) => {
        if (point.setWinner) return index;
        return -1;
      })
